...
test('teach mode steps forward through all six demos via buttons', async ({ page }) => {
  ...
  for (let i = 0; i < teachSteps.length; i += 1) {
    await expect(counter).toHaveText(
      `Demo ${i + 1}/${teachSteps.length} — ${STRINGS.en[teachSteps[i].titleKey]}`,
    )
...
test('teach mode steps backward via arrow keys and wraps', async ({ page }) => {
  ...
  await expect(counter).toHaveText(
    `Demo ${teachSteps.length}/${teachSteps.length} — ${STRINGS.en[teachSteps[teachSteps.length - 1].titleKey]}`,
  )

  await page.keyboard.press('ArrowRight')
  await expect(counter).toHaveText(
    `Demo 1/${teachSteps.length} — ${STRINGS.en[teachSteps[0].titleKey]}`,
  )
})
