...
test("a subresource fetch is served from cache while the network is offline", async ({ page, context }, testInfo) => {
  ...
  test.skip(testInfo.project.name === "webkit-mobile", "WebKit offline emulation blocks fetch() even when a service worker controls the page");
  await page.goto("/");
  await waitForServiceWorker(page);
