...
# Simple (non-list) TEXT["en"] keys that terminology-map.json can overlay.
# m6_levers is deliberately excluded — it is sourced from spine-s1.json's
# "levers" array (English-only, generated by export-spine.mjs), not from TEXT.
_SIMPLE_TEXT_KEYS = [
    "cold_open_title", "cold_open_body", "m1_title", "m1_body",
    "m2a_title", "m2a_body", "m2b_title", "m2b_body",
    "m3_title", "m3_body", "m3_app", "m4_title", "m4_body",
    "m5_title", "m5_body", "m5_exercise", "m6_decoder_title",
    "m6_levers_title", "close_title", "close_body",
]
...
def load_text(lang):
    ...
    dict (byte-stable, never touches terminology-map.json). For "vi"/"zh",
    overlays translations from assets/teaching/terminology-map.json onto a
    copy of TEXT["en"], and aborts (PHASE-05 TASK-05-02) listing every key
    that is still UNTRANSLATED or missing from the map, rather than silently
    falling back to English."""
    base = TEXT["en"]
    if lang == "en":
        return base

    with open(os.path.join(ASSETS, "terminology-map.json"), encoding="utf-8") as f:
        term_map = json.load(f)["entries"]

    def get(map_key):
        entry = term_map.get(map_key)
        if entry is None:
            return None
    ...
    for list_key, count in _LIST_TEXT_KEYS.items():
        new_list = list(base[list_key])
        for i in range(count):
            map_key = f"{list_key}.{i}"
            value = get(map_key)
            if value is None:
                missing.append(map_key)
            else:
                new_list[i] = value
        result[list_key] = new_list

    new_rows = []
    for i, (plain, symbol) in enumerate(base["m6_decoder_rows"]):
        map_key = f"m6_decoder_rows.{i}.plain"
        value = get(map_key)
        if value is None:
            missing.append(map_key)
            new_rows.append((plain, symbol))
        else:
            new_rows.append((value, symbol))
    result["m6_decoder_rows"] = new_rows

    if missing:
        print(f"ERROR: cannot build --lang {lang} — {len(missing)} terminology-map.json "
              f"entries are UNTRANSLATED or missing (translate them in "
              f"assets/teaching/terminology-map.json, then retry):", file=sys.stderr)
        for key in missing:
