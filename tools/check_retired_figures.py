        ...
        for line_no, line in enumerate(lines, start=1):
            ...
            for entry in retired:
                ...
                if needle in lowered:
                    ...
                    violations.append(
                        f'RETIRED-FIGURE {rel}:{line_no}: "{entry["text"]}" '
...
def main(argv: list[str] | None = None) -> int:
    ...
    violations = scan_files(REPO_ROOT, config)
    ...
    n_files = len(_scanned_files(REPO_ROOT, config.get("scan", [])))
    print(f"RETIRED-FIGURES PASS ({n_files} files scanned)")
    return 0
