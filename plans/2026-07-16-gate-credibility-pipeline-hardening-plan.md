...
date: "2026-07-16"
status: "superseded — 2026-08-29: at the user's direction, closed out in favor of plans/2026-08-22-delivery-stall-recovery-plan.md, which is cited as this plan's successor. That plan only implemented 12 of this plan's 47 tasks (the extended STRIKES grid, grid-size parameterization, compare_deck.py, the deck-build CI job, requirements.txt-based pip install, --out, the dead audit-loop deletion, tools/tests in CI, the archived one-offs README, the trilingual string table, ?lang= routing, and URL state). The remaining 35 — the sensitivity band/computeBand and derived lender threshold, the deck-figures.json manifest and positional verifier, EXTRA_ALLOWED removal, the tools/ reorganization and root README, the 14 hardcoded -en deck asset paths, the 66 remaining UNTRANSLATED terminology-map entries, and the M6 sensitivity panel — are NOT covered by any plan as of this closure and are not tracked "anywhere going forward unless a new plan is opened for them."
